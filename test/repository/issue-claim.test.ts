...
import {
  ...
  reconcileIssueClaim,
  resolveIssueClaim,
} from "../../.github/scripts/issue-claim.mjs"
...
describe("resolveIssueClaim", () => {
  test("preserves the earliest eligible claim", async () => {
    expect(await resolve([comment(1, "CLAIM", "alice"), comment(2, "CLAIM", "bob")])).toEqual([
      "alice",
    ])
  })
  ...
  test("only the owner can release a claim", async () => {
    expect(
      await resolve([
        comment(1, "CLAIM", "alice"),
        comment(2, "UNCLAIM", "bob"),
        comment(3, "UNCLAIM", "alice"),
      ]),
    ).toEqual([])
  })
  ...
  test("clears assignment for an authorized clear command", async () => {
    expect(
      await resolve([comment(1, "CLAIM", "alice"), comment(2, "CLEAR", "maintainer")]),
    ).toEqual([])
  })

  test("treats assignment as a claim and preserves unrelated assignees", async () => {
    expect(
      await resolve([assignment(1, "assigned", "carol"), comment(2, "CLAIM", "alice")]),
    ).toEqual(["carol"])
    expect(
      await resolve([
        comment(1, "CLAIM", "alice"),
...
test("issue claim workflow is narrow, serialized, and least privilege", async () => {
  const workflow = await readFile(
    new URL("../../.github/workflows/issue-claim.yml", import.meta.url),
    "utf8",
  )
